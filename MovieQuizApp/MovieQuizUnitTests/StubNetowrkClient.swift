...
struct StubNetowrkClient: NWServiceProtocol {
    ...
    func fetchMovies(_ handler: @escaping (Result<Data, any Error>) -> Void) {
        if emulateError {
            handler(.failure(TestError.test))
        } else {
            handler(.success(expectedMoviesResponse))
        }
    }
    ...
    func fetchImage(url: URL, handler: @escaping (Result<Data, any Error>) -> Void) {
        if emulateError {
            ...
            handler(.success(expectedImageResponse))
        }
    }
    
    private var expectedImageResponse: Data {
        Data()
    }
    
    private var expectedMoviesResponse: Data {
        """
        {
           "errorMessage" : "",
           "items" : [
